...
`driver_relevance`、`tumor_specificity`、`normal_hla_ligand_overlap`这三个真实计算逻辑背后，各自依赖一份参考数据库：

| 参考数据库 | 数据本身 | 需要手动配置吗 |
|---|---|---|
| 驱动基因表 | `resources/driver_genes.tsv`仍是我们给的111行精简示例，真实数据没有捆绑在仓库里 | **是**：去 [OncoKB cancerGeneList](https://www.oncokb.org/cancer-genes)（需注册申请）下载真实数据，然后在profile TOML里配置 `[driver_genes] reference_path = "resources/真实文件名"` |
| GTEx表达矩阵 | `resources/GTEx_gene_median_tpm.gct`（74,631行，官方GCT格式，真实数据） | 不需要，默认路径已经指向这份真实文件 |
| 正常HLA配体库 | `resources/normal_hla_ligands.tsv`（694,505行，真实IEDB清洗数据） | 不需要，默认路径已经指向这份真实文件 |

也就是说，目前这三项里只有**驱动基因表**还需要用户自己准备，GTEx和正常HLA配体库都已经是开箱即用的真实数据。
...
**normal_hla_ligand_overlap**——默认路径已经指向真实IEDB数据（694,505条），一般不需要额外确认；如果这次命令显式传了别的`--normal-hla-ligands`路径，检查一下那份文件是不是也是真实数据而不是随手指了个小的占位文件——毕竟这项安全检查的有效性完全取决于参考库里有多少真实条目，库太小基本不会有任何候选命中"跟已知正常配体重叠"，不能把"没有重叠"直接解读成"这条肽真的很新"。

**persistence**——这个字段目前**没有真实计算逻辑**，全部adapter恒为`"0.5"`，在`event_weights`里权重0.15。跟前面几个不同的是，这个字段暂时也没有一个明确的公开数据源能替代（它代表的"MRD持续追踪"需要纵向随访数据，不是查个表就能补全的）。**遇到这个字段直接如实告诉用户"这一项目前是恒定占位值，`event_score`里这部分权重实际不产生区分度"，不需要教用户怎么验证真假，因为目前就是全库统一的占位符，没有例外**。
...
- `normal_hla_ligand_overlap`默认已经用的是真实IEDB数据，不需要额外建议；如果发现这次实际用的是个明显很小的自定义文件，提醒用户这项安全检查参考力度不够，建议换回默认的`resources/normal_hla_ligands.tsv`或者自己准备更大的参考库
- `persistence`目前全库恒定，不用建议用户去"修复"它，如实说明"这一项暂时没有可靠的公开数据源可以替代，权重贡献可以忽略"即可
