...
这个skill覆盖五步，**必须按顺序执行，不要跳过前几步直接跑排序**：

0. 先清点用户给的目录/文件里实际有哪些辅助证据，判断这次能不能完成"全套真实数据打分"，哪些会退化成默认值——**如果发现已经有现成的排序表，直接进入第五步解读，不要重新跑**
...
### 情况A：`scoring/ranked_peptides.v03.tsv` 已经存在
