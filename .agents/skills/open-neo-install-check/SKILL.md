...
7. Re-run discovery after installation, pin production runtime entrypoints, execute minimal smoke checks, and publish generated machine-local manifests under `configs/local/`. The runtime pinning records the local FACETS R environment and `snp-pileup`, Sequenza chr FASTA/GC wiggle, HMFTOOLS/PURPLE suite paths, SpecHLA `samtools`, and a project-local `bedtools` wrapper for LOHHLA when the portable bedtools package is present. For EasyFuse, Skill1 must also ensure v2 installations with legacy module references have `environments/easyfuse_src.yml` and `environments/requantification_wo_easyfuse.yml`, pin `NXF_CONDA_CACHEDIR` to an absolute project work cache, and avoid conflicting conda/mamba yes flags before production runs. Licensed tools are configured only when the user has supplied a legal local installation.
