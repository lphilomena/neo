...
- Full installs must leave FACETS, Sequenza, PURPLE/HMFTOOLS, SpecHLA, and LOHHLA callable without relying on interactive shell state. `conf/tools.env.local.sh` receives a marked install-check block for these paths, and `bin/bedtools` is generated when the portable bedtools binary requires the bundled Conda C++ runtime. Re-running `install` or `resume` replaces only the marked block/wrapper and skips already synchronized assets when their checkpoints and markers are still valid.
