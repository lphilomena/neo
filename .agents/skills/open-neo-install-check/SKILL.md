...
- Full installs must leave FACETS, Sequenza, PURPLE/HMFTOOLS, SpecHLA, LOHHLA, and fusion callers callable without relying on interactive shell state. `conf/tools.env.local.sh` receives a marked install-check block for these paths, and `bin/bedtools` is generated when the portable bedtools binary requires the bundled Conda C++ runtime. EasyFuse readiness requires its reference marker, Nextflow entrypoint, compatibility env YAML files when referenced by installed modules, deployed tool-directory fallback, an absolute Nextflow conda cache, and an explicit STAR index override to `starfusion_index/ref_genome.fa.star.idx` with both `Genome` and `genomeParameters.txt` readable. STAR-Fusion readiness requires the CTAT/EasyFuse starfusion index, deployed executable fallback, compatible Perl invocation for `Set::IntervalTree`, `vendor_perl` in `PERL5LIB` for modules such as `common::sense`, a STAR executable on PATH, and conservative default threads for reruns. Arriba readiness requires fixed blacklist assets, fixed reference lookup, and correct Arriba 2.5.1 argument mapping (`-O` for discarded output, `-p` for protein domains). Re-running `install` or `resume` replaces only the marked block/wrapper and skips already synchronized assets when their checkpoints and markers are still valid.
