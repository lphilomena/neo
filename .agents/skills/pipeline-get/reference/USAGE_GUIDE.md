...
仍然保留，给出**全部**已注册工具（`src/neoag_v03/tools/registry.py`里定义的完整工具集）的可用性总览，
适合部署/CI阶段一次性核对整套环境。`conf/tools.toml`现已扩充到24个工具段，覆盖
`docs/INSTALL_AND_DATA.md`"External Tool Installation Table"里列出的全部工具
（pVACtools/NetMHCpan/NetMHCstabpan/MHCflurry/PRIME/MixMHCpred/BigMHC/DeepImmuno/VEP/GATK4/
EasyFuse系/Manta系/LOHHLA/Polysolver/FACETS/ASCAT(含v3)/PyClone-VI/参考数据等）。**注意**：
`check-tools`和 `run-demo --entry-mode`目前仍是两套独立的工具清单（前者读 `tools/registry.py`，
后者读 `conf/tools.toml`），尚未合并成一份——这是已知的后续优化项，日常按入口开发调试建议优先用
`run-demo --entry-mode`。
