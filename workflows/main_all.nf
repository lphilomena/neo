/*
 * NeoAg — Complete End-to-End Pipeline (HLA Typing + Variant Calling + Scoring)
 *
 * Full pipeline from raw BAM files to ranked neoantigen report:
 *   1. OPTITYPE                  — de-novo HLA typing from normal blood BAM
 *   2. GATK_MUTECT2              — somatic variant calling (auto-indexes BAMs)
 ...
 *   6. NEOAG_V03_RC              — scoring chain → ranked report
 *
 * Required parameters:
...
workflow {
  ...
  // WGS mode: no capture intervals — pass empty string to skip -L
  ...
  // === Stage 2 — Somatic variant calling (GATK Mutect2, auto-indexes BAMs) ==
  GATK_MUTECT2(
    sample_id,
    tumor_bam,
    normal_bam,
  ...
  // Read OptiType hla_alleles.txt and convert to list for SNV_WRITE_RUN_CONFIG.
  ...
  // === Stage 6 — Derive channels from upstream output ======================
