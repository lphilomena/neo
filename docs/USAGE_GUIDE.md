...
| 2 | `neoag-v03 extract-variant-peptides --input-vcf <annotated_vcf> --output <outdir>/upstream/tools/variant_peptides.tsv --hla-alleles <hla> --tumor-sample-name <name>` | `variant_peptides.tsv` | 非空 |
| 3 | `neoag-v03 peptide-predict -i <variant_peptides.tsv> -o <outdir>/presentation` | `presentation/presentation_evidence.tsv` 等 | 非空 |
...
只打印并验证**该入口**用到的工具，用仓库自带fixture跑一遍完整链路，不需要真实安装重量级工具即可验证代码本身可用。
这是推荐的日常验收方式。
...
仍然保留，给出**全部**已注册工具（pVACtools/NetMHCpan/MHCflurry/VEP/GATK4/LOHHLA/FACETS/ASCAT/PyClone-VI/
STAR-Fusion系工具/Manta系工具等）的可用性总览，适合部署/CI阶段一次性核对整套环境，日常按入口开发调试建议
优先用`run-demo --entry-mode`。
...
| `--cache-dir` | VEP cache目录，默认`$NEOAG_VEP_CACHE`或`~/.vep` |
| `--plugins-dir` | VEP_plugins目录，默认`work/vep_plugins` |
| `--fork` | 并行度 |
