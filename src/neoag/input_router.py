...
from .utils import copy_if_different, read_tsv, write_tsv
...
def build_raw_intermediates(
    ...
    *,
    ...
    if pre_events and pre_peptides:
        copy_if_different(pre_events, raw_events)
        copy_if_different(pre_peptides, raw_peptides)
        return {
    ...
    if mode == "sv":
        ...
        if not (sv_events and sv_peptides):
            raise ValueError(
                ...
                "(from neoag sv-build-raw / sv-run-full)"
            )
        copy_if_different(sv_events, raw_events)
        copy_if_different(sv_peptides, raw_peptides)
