...
def run_pipeline_full(
    *,
    ...
    if run_demo_for_fixture:
        ...
        res = run_command([sys.executable, "-m", "neoag.cli", "run-demo", "--sample-id", sample_id or "PIPELINE", "--outdir", str(demo_out)], cwd=project_root, timeout=300, logger=logger, risk_level="LOW", allow_execute=not dry_run)
