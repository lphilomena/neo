...
def pick_csq_transcript(
    ...
    *,
    ...
    for parts in csq_entries:
        ...
        # Accept plugin-backed protein context, frameshift context, or at
        # minimum a simple amino-acid substitution for VEP-only missense calls.
...
def build_mutant_protein(
    ...
    *,
    ...
    if not wt:
        # If VEP plugin protein context is absent, synthesize a minimal
        # backbone from Amino_acids so missense/inframe calls can still yield
        # candidate k-mers. This is a fallback, not a replacement for plugin
        # backed full protein context.
    ...
    with _open_vcf(path) as fh:
        for line in fh:
            ...
            if line.startswith("#"):
                continue
            if not csq_idx:
                raise ValueError(f"No CSQ annotations in {path}; run neoag vep-annotate first")
