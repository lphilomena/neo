...
from neoag.agents.skill_router import find_named_files
...
from .schema_validation import SchemaValidationError, validate_plan_object, validate_skill_results
...
from .schemas import CaseState, CoordinatorPlan, ExecutionMode, IntentResult
from .skill_executor import execute_plan, load_registry
from .summarizer import summarize_with_llm



def _write_schema_failure(outdir: Path, case_id: str, message: str, mode: ExecutionMode, error: SchemaValidationError, audit_path: Path, project_root: str) -> None:
    nl = chr(10)
    payload = {"case_id": case_id, "status": "FAIL", "error": error.to_dict()}
    (outdir / "schema_validation_error.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + nl, encoding="utf-8")
    issue_lines = nl.join(f"- {x}" for x in error.issues)
    final_text = (
        "# LLM-assisted Coordinator Failed" + nl + nl +
        "LLM/tool JSON schema validation failed. The task was stopped before executing any skill." + nl + nl +
        f"Schema: {error.schema_name}" + nl + nl +
        "## Issues" + nl + issue_lines + nl
    )
    (outdir / "final_response.md").write_text(final_text, encoding="utf-8")
    fallback_intent = IntentResult(intent="unknown", confidence=0.0, user_goal=message, source="schema_validation")
    empty_plan = CoordinatorPlan(plan_id=f"schema_failed_{int(time.time())}", goal=message[:240], intent="unknown", steps=[], model_provider="schema_validation")
    append_audit(audit_path, {"ts": time.time(), "event": "schema_validation_failed", "error": error.to_dict()})
    append_task_event(project_root, outdir, {"event": "schema_validation_failed", "case_id": case_id, "status": "FAIL", "error": error.to_dict(), "outdir": str(outdir)})
    state = CaseState(
        case_id=case_id,
        message=message,
        mode=mode,
        intent=fallback_intent,
        input_state=build_input_state([], None, outdir=outdir, execute_input_qc=False),
        plan=empty_plan,
        skill_results=[],
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        (outdir / "task_spec.json").write_text(json.dumps(parsed_task.to_dict(), ensure_ascii=False, indent=2) + chr(10), encoding="utf-8")
        ...
        validate_plan_object(plan, tool_registry)
    except SchemaValidationError as e:
        _write_schema_failure(outdir, case_id, args.message, args.mode, e, audit_path, args.project_root)
        print(json.dumps({"case_id": case_id, "status": "FAIL", "outdir": str(outdir), "schema_validation_error": str(outdir / "schema_validation_error.json"), "final_response": str(outdir / "final_response.md")}, ensure_ascii=False, indent=2))
        return 1
    ...
    try:
        validate_skill_results(results)
    except SchemaValidationError as e:
        _write_schema_failure(outdir, case_id, args.message, args.mode, e, audit_path, args.project_root)
        print(json.dumps({"case_id": case_id, "status": "FAIL", "outdir": str(outdir), "schema_validation_error": str(outdir / "schema_validation_error.json"), "final_response": str(outdir / "final_response.md")}, ensure_ascii=False, indent=2))
        return 1
