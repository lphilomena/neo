...
def run_tool_reference_qc(args: dict[str, Any]) -> dict[str, Any]:
    from neoag.agent_skills.tool_reference_qc import main as tool_qc_main
    outdir = ensure_dir(args["outdir"])
...
def run_demo_smoke(args: dict[str, Any]) -> dict[str, Any]:
    from neoag.agent_skills.run_demo_smoke import main as run_demo_main
    outdir = ensure_dir(args["outdir"])
    argv = ["--project-root", str(args.get("project_root") or "."), "--outdir", str(outdir)]
...
def run_release_qc(args: dict[str, Any]) -> dict[str, Any]:
    from neoag.controlled_execution.release_audit import scan_release_boundary, write_release_audit
    outdir = ensure_dir(args["outdir"])
...
def run_hpc_runner(args: dict[str, Any]) -> dict[str, Any]:
    outdir = ensure_dir(args["outdir"])
    approved = bool(args.get("approved") or False)
    cmd = str(args.get("command") or "neoag pipeline-full --sample-manifest sample.yaml --profile slurm")
