...
SKILL_SPECS: list[SkillSpec] = [
    ...
    _spec(
        ...
        description="Detect raw DNA/RNA or precomputed inputs, probe callable tools/references, generate an auditable production DAG, execute selected primary and cross-validation tools, and emit weighted plus evidence-consensus rankings.",
        use_when=["外部 Agent 需要从原始 FASTQ/BAM 或结果文件运行完整分析", "需要按可用工具和参考库自动生成 production manifest", "需要自动识别多入口并生成 all_tool_results 与双排序", "需要复用已有结果补生成 evidence-consensus"],
        do_not_use_when=["只需解释 event-level 结果和生成报告，应使用 open-neo-review"],
        required_inputs=["sample_manifest，或受支持的输入文件/已有 result_dir"],
        optional_inputs=["mode", "approved", "tools_manifest", "reference_manifest", "automatic_tool_policy", "rules", "production_manifest", "stub"],
        outputs=["input_status.json", "route_plan.json", "capability_decisions.tsv", "capability_aware.production.toml", "all_tool_results.tsv", "ranked_peptides.weighted_baseline.tsv", "ranked_peptides.evidence_consensus.tsv", "ranked_events.evidence_consensus.tsv", "ranking_compare_weighted_vs_consensus.md", "run_manifest.json"],
