...
def effective_weights(profile: Mapping[str, Any], peptide: Mapping[str, Any], presentation: Mapping[str, Any] | None = None) -> dict[str, float]:
    """Redistribute immunogenicity weight when only the pVAC placeholder is available."""
    w = {k: float(v) for k, v in profile.get("score_weights", {}).items()}
...
def compute_peptide_efficacy(
    ...
    *,
    ...
    w = effective_weights(profile, peptide, presentation)
