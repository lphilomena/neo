...
def main(argv: list[str] | None = None) -> int:
    ...
    append_audit(audit_path, {"ts": time.time(), "event": "intent", "intent": intent.to_dict()})

    registry = load_registry(args.project_root)
    plan = plan_with_llm(args.message, intent, input_state, registry, provider)
    ...
    if args.mode == "plan":
        results = execute_plan(plan, files, args.result_dir, outdir, args.project_root, mode="plan", allow_high_risk=False)
    else:
        allow = args.mode == "execute-with-approval" and args.allow_high_risk
        results = execute_plan(plan, files, args.result_dir, outdir, args.project_root, mode=args.mode, allow_high_risk=allow)
