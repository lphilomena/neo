...
SKILL_SPECS: list[SkillSpec] = [
    ...
    _spec(
        ...
        use_when=["需要生成 final_priority 和 recommendation ranked peptides"],
        do_not_use_when=["只需比较两个排序文件时，使用 neoag-ranking-compare"],
        required_inputs=["raw_peptides", "presentation"], optional_inputs=["expression", "rna_evidence", "ccf", "appm", "safety"],
        outputs=["ranked_peptides.recommendation.tsv", "ranked_peptides.netmhcpan42.tsv", "ranked_events.tsv", "validation_plan.tsv"], risk_level="LOW",
    ),
    # C: Review/report/design
    _spec(
        name="neoag-ranking-compare", category="C", handler="ranking_compare",
        purpose="NetMHCpan42 与综合推荐排序比较",
        description="Compute overlap, rank shifts and interpretation between NetMHCpan42 and recommendation ranking tables.",
        use_when=["用户问两个排序文件差异", "需要解释 NetMHCpan42 top fusion 为什么被综合降权"],
        do_not_use_when=["需要生成 ranking 时，使用 neoag-ranking"],
        required_inputs=["recommendation", "netmhcpan42"], outputs=["ranking_compare_report.md", "topn_overlap.tsv", "rank_shift.tsv"], risk_level="LOW",
    ),
    _spec(
