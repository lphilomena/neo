...
def _run_mini_smoke(project_root: Path, outdir: Path, logger: AuditLogger) -> list[CheckRow]:
    ...
    modules = [
        "neoag_v03.cli",
        "neoag_v03.controlled_execution.doctor",
        "neoag_v03.controlled_execution.release_audit",
        "neoag_v03.controlled_execution.pipeline_runner",
    ]
    ...
    for label, cmd in {
        "neoag_v03_cli_help": [sys.executable, "-m", "neoag_v03.cli", "--help"],
        "controlled_execution_doctor_help": [sys.executable, "-m", "neoag_v03.controlled_execution.doctor", "--help"],
        "controlled_execution_release_audit_help": [sys.executable, "-m", "neoag_v03.controlled_execution.release_audit", "--help"],
    }.items():
...
def _run_core_smoke(project_root: Path, outdir: Path, logger: AuditLogger, *, run_demo: bool, run_pytest: bool, run_nextflow: bool, allow_execute: bool) -> list[CheckRow]:
    ...
    if run_demo:
        ...
        res = run_command([sys.executable, "-m", "neoag_v03.cli", "run-demo", "--sample-id", "DOCTOR", "--outdir", str(demo_out)], cwd=project_root, timeout=300, logger=logger, risk_level="LOW", allow_execute=allow_execute)
...
def run_doctor(
    *,
    ...
    try:
        import neoag_v03  # noqa: F401
        rows.append(_row("core", "python_import_neoag_v03", "OK", "neoag_v03 importable"))
    except Exception as exc:
        rows.append(_row("core", "python_import_neoag_v03", "FAIL", str(exc), blocking=True))
    for name in ["python", "neoag-v03", "neoag-agent", "neoag-llm-agent"]:
    ...
    for name, exe in sorted(_flatten_tools(tools_data).items()):
        if name in {"python", "neoag-v03", "neoag-agent", "neoag-llm-agent"}:
            continue
