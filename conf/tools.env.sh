...
if [[ -x "${NEOAG_CONDA_BASE}/envs/${NEOAG_VEP_ENV}/bin/vep" ]]; then
...
if [[ -x "/home/na/netMHCpan42/netMHCpan" ]]; then
  export NETMHCPAN_HOME="/home/na/netMHCpan42"
elif [[ ! -x "${NETMHCPAN_HOME}/netMHCpan" && -n "${NEOAG_TOOL_QUARANTINE}" && -x "${NEOAG_TOOL_QUARANTINE}/netMHCpan/netMHCpan" ]]; then
