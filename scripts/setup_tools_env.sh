...
ENV_PREFIX="$(conda_safe env list | awk -v n="${ENV_NAME}" '$1==n {print $NF; exit}')"
...
if [[ ! -x "${ENV_PREFIX}/bin/pvacseq" ]]; then
  echo "==> Installing pvactools via pip ..."
  "${PYTHON_BIN}" -m pip install "pvactools>=6.1,<7"
fi

echo "==> Ensuring setuptools<82 (pkg_resources required by mhcflurry) ..."
"${PYTHON_BIN}" -m pip install -q "setuptools>=65,<82"

echo "==> Ensuring Biopython ..."
"${PYTHON_BIN}" -m pip install -q biopython

echo "==> Ensuring tf-keras legacy shim for MHCflurry ..."
TF_KERAS_SPEC="$("${PYTHON_BIN}" - <<'PY'
import tensorflow as tf
major, minor, *_ = tf.__version__.split(".")
print(f"tf-keras>={major}.{minor},<{major}.{int(minor) + 1}")
PY
)"
"${PYTHON_BIN}" -m pip install -q "${TF_KERAS_SPEC}"

echo "==> Fetching MHCflurry models..."
if [[ -x "${ENV_PREFIX}/bin/mhcflurry-downloads" ]]; then
  "${ENV_PREFIX}/bin/mhcflurry-downloads" fetch || { echo "WARN: mhcflurry-downloads fetch failed. Try: LD_LIBRARY_PATH=\"${ENV_PREFIX}/lib:${LD_LIBRARY_PATH:-}\" ${ENV_PREFIX}/bin/mhcflurry-downloads fetch"; }
else
  echo "WARN: mhcflurry-downloads missing from ${ENV_PREFIX}/bin"
fi

if [[ ! -x "${ENV_PREFIX}/bin/vep" ]]; then
...
# Generated by scripts/setup_tools_env.sh — source before neoag run-full
...
echo ""
echo "Verify:"
echo "  source ${TOOLS_ENV_SH} && neoag check-tools"
echo ""
...
for cmd in pvacseq pvacfuse mhcflurry-predict vep; do
  if [[ -x "${ENV_PREFIX}/bin/${cmd}" ]]; then
    echo "  OK  $cmd -> ${ENV_PREFIX}/bin/${cmd}"
  else
    echo "  MISS $cmd"
  fi
done
